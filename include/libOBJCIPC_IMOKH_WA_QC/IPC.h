...
@interface OBJCIPC_IMOKH_WA_QC : NSObject {
...
// SpringBoard* -----> App
+ (BOOL)sendMessageToAppWithIdentifier:(NSString *)identifier messageName:(NSString *)messageName dictionary:(NSDictionary *)dictionary replyHandler:(OBJCIPC_IMOKH_WA_QCReplyHandler)handler;

// App* -----> SpringBoard
+ (BOOL)sendMessageToSpringBoardWithMessageName:(NSString *)messageName dictionary:(NSDictionary *)dictionary replyHandler:(OBJCIPC_IMOKH_WA_QCReplyHandler)handler;
...
// App -----> SpringBoard*
+ (void)registerIncomingMessageFromAppHandlerForMessageName:(NSString *)messageName handler:(OBJCIPC_IMOKH_WA_QCIncomingMessageHandler)handler;
...
+ (void)registerIncomingMessageHandlerForAppWithIdentifier:(NSString *)identifier andMessageName:(NSString *)messageName handler:(OBJCIPC_IMOKH_WA_QCIncomingMessageHandler)handler;
+ (void)unregisterIncomingMessageHandlerForAppWithIdentifier:(NSString *)identifier andMessageName:(NSString *)messageName;

// SpringBoard -----> App*
+ (void)registerIncomingMessageFromSpringBoardHandlerForMessageName:(NSString *)messageName handler:(OBJCIPC_IMOKH_WA_QCIncomingMessageHandler)handler;
...
// manage connections
- (OBJCIPC_IMOKH_WA_QCConnection *)activeConnectionWithAppWithIdentifier:(NSString *)identifier;
- (void)addPendingConnection:(OBJCIPC_IMOKH_WA_QCConnection *)connection;
- (void)notifyConnectionBecomesActive:(OBJCIPC_IMOKH_WA_QCConnection *)connection;
- (void)notifyConnectionIsClosed:(OBJCIPC_IMOKH_WA_QCConnection *)connection;
- (void)removeConnection:(OBJCIPC_IMOKH_WA_QCConnection *)connection;

// queue up the outgoing message
- (void)queueOutgoingMessage:(OBJCIPC_IMOKH_WA_QCMessage *)message forAppWithIdentifier:(NSString *)identifier;
